...
class ContractCustomPlan(models.Model):
    ...
    def set_user_data(self, token=False):
        for obj in self:
            ...
            """
            extra_modules = self.env['saas.module'].sudo().search([('is_published', '=', True), ('id', 'not in', obj.saas_module_ids.ids)])
            ...
            if obj.is_custom_plan:
                ...
                data['user_id'] = obj.odoo_version_id.template_user_id and int(obj.odoo_version_id.template_user_id)
                data['user_data'] = dict(
