...
class odoo_container:
    def  __init__(self,db="dummy",odoo_image="odoo:12.5",odoo_config = None,host_server = None, db_server = None, version = "15.0"):
...
def main(context=None):
    ...
    if odoo_version in ["11.0","12.0","13.0","14.0","15.0"]:
    ...
    response = saas_client_db.create_saas_client(operation = "install", odoo_url="http://{}:{}".format(endpoint, saas_port),odoo_username = context.get("login") ,odoo_password = context.get("password"), database_name = db ,modules_list = modules,admin_passwd = context.get("password"))
