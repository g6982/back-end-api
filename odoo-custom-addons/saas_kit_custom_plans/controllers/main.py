...
class PlanPage(http.Controller):
    ...
    def _create_custom_plan_data(self):
        ...
        data['odoo_version'] = request.env['saas.odoo.version'].sudo().search([('state', '=', 'confirm')]) if data['is_odoo_version'] else request.env['saas.odoo.version'].sudo().search([('state', '=', 'confirm'), ('code', '=', '15.0')], limit=1)
